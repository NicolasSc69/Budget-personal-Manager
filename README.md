...
- **Projected balance** ("Solde prévisionnel approximatif" in French) — shows two figures, both excluding transactions whose tag has **Excluded from forecast** checked (`Category::$excludedFromForecast`, `TransactionRepository::sumFiltered(..., excludeForecastExcluded: true)`) so the account's declared average salary isn't double-counted alongside the real salary transaction. Only shown when a single, non-savings account is selected — it's hidden in the aggregate "all accounts" view and for savings accounts, where it isn't meaningful.
  - **Main figure** — `current balance + upcoming transactions still due this month` (`TransactionRepository::sumUntil()` plus `TransactionRepository::sumFiltered(currentMonthOnly: true, upcomingOnly: true, excludeForecastExcluded: true)`). Deliberately excludes transactions already dated this month but before today — those are already included in "current balance" — to avoid double-counting them.
  - **Theoretical figure** (shown in parentheses, labelled "Théorique") — `Account::$averageSalary` (or the sum across all accounts in the aggregate view) plus the *whole* current month's balance, past and future (`TransactionRepository::sumFiltered(currentMonthOnly: true, excludeForecastExcluded: true)`). An older, coarser estimate based on the declared average salary rather than the account's actual current balance; kept alongside the main figure as a sanity check.
  - The **Excluded from forecast** flag on a tag is checked against the transaction's own tag only, not inherited by child tags (unlike most other tag filters in the app, which include descendants via `Category::getSelfAndDescendantIds()`) — tagging real salary/bonus transactions with an unflagged sub-tag of an otherwise-excluded parent tag will still double-count them here.
...
The daily balance chart's period (`DashboardController::resolveDailyBalancePeriod()`) is not a fixed calendar year — it adapts to the account's actual transaction history (globally across all accounts when none is selected), via `TransactionRepository::findEarliestDate()` / `findLatestDate()`: if the account's whole history spans less than 365 days, the chart shows it in full (first operation to last operation, without padding into an empty future); otherwise it shows a 365-day window centered on the midpoint between the first and last operation. The chart's caption above it (`dailyBalanceChartPeriodLabel`, translation key `Balance trend from %start% to %end%`) is built from that same resolved period, so it always matches what's actually plotted.
